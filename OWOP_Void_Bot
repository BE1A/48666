...
window.addEventListener('keyup', function(e)
{
    ...
    {
        GM_setValue("spam", !GM_getValue("spam"));
        //spam = !spam;
    }
    else if(key == 87)
    {
...
function loop()
{
    ...
    if(!undefPut || typeof put != "undefined")
    {
        if(preRender === null)
        {
            ...
            for(var rgbFlip = 0; rgbFlip < 256; rgbFlip++)
            {
                rArray[rgbFlip] = 0;
                gArray[rgbFlip] = 0;
                bArray[rgbFlip] = 0;
        ...
        if(document.hasFocus())
        {
            ...
            GM_setValue("last", lastMap);
        }
        spam = GM_getValue("spam");
        var doClick = false;
        if(true)
        {
            ...
            pixly = mainpixly;
            var found = false;
            for(var inci = 0; inci < 256 && !found && spam; inci++)
            {
                var inciP = inci;
                if(randRev == -1)
                {
